...
//
  ...
  var validateBearerToken = function(accessToken, refreshToken, profile, done) {
    if (config.debug) console.log('in validateBearerToken');
    if (!accessToken) {
  ...
  var setupBearer = function(path, app) {
    ...
    if (config.debug) { console.log('in setupBearer') }
    passport.use(bearer);
  ...
  var ensureAuthenticated = function(req, res, next) {
    ...
    if (req.isAuthenticated()) {
      if (config.debug) { console.log('authenticated by request session.')}
      return next();
    }
    if (config.debug) { console.log('not authenticated by request session.')}
    if (useSession) {
  ...
  var userForRequest = function (req) {
    ...
    if (config.debug) console.log('request user is: ', user.name);
