...
var config = require('yargs')
  ...
  .options({
    'listen-port': {
      ...
      describe: 'Transform name(s) to apply to the request/response after authentication [choices: "user_header", "token_header", "kibana_es", "none"]',
  ...
  .check(function(args) {
    ...
    (typeof(args.transform) === 'string' ? args.transform.split(',') : args.transform).forEach(function(transform) {
      if (['user_header', 'token_header', 'forward_user', 'kibana_es', 'none'].indexOf(transform) === -1)
